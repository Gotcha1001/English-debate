...
/**
 * Glue between <LessonHeader> and Convex for one Idioms lesson: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation). Same shape as
 * useQuickQuestionHeader, pointed at the `idiomSets` table.
 */
export function useIdiomHeader(id: Id<"idiomSets">) {
  const uploadHeaderImage = useAction(api.idiomsActions.uploadHeaderImage);
  const removeHeaderImage = useAction(api.idiomsActions.removeHeaderImage);
  const updateLearningObjective = useMutation(
    api.idiomsData.updateLearningObjective,
  );
