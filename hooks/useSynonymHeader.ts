...
/**
 * Glue between <LessonHeader> and Convex for one Synonym Spectrum set: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation). Same shape as
 * useQuickQuestionHeader, pointed at the `synonymSets` table.
 */
export function useSynonymHeader(id: Id<"synonymSets">) {
  const uploadHeaderImage = useAction(api.synonymsActions.uploadHeaderImage);
  const removeHeaderImage = useAction(api.synonymsActions.removeHeaderImage);
  const updateLearningObjective = useMutation(
    api.synonymsData.updateLearningObjective,
  );
