...
/**
 * Glue between <LessonHeader> and Convex for one Word Relations set: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation). Same shape as
 * useGrammarHeader / useIdiomHeader, pointed at the `wordRelationSets` table.
 */
export function useWordRelationHeader(id: Id<"wordRelationSets">) {
  const uploadHeaderImage = useAction(
    api.wordRelationsActions.uploadHeaderImage,
  );
  const removeHeaderImage = useAction(
    api.wordRelationsActions.removeHeaderImage,
  );
  const updateLearningObjective = useMutation(
    api.wordRelationsData.updateLearningObjective,
  );
