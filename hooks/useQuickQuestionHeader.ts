...
/**
 * Glue between <LessonHeader> and Convex for one question set: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation).
 */
export function useQuickQuestionHeader(id: Id<"quickQuestionSets">) {
  const uploadHeaderImage = useAction(
    api.quickQuestionsActions.uploadHeaderImage,
  );
  const removeHeaderImage = useAction(
    api.quickQuestionsActions.removeHeaderImage,
  );
  const updateLearningObjective = useMutation(
    api.quickQuestionsData.updateLearningObjective,
  );
  ...
  const saveObjective = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        // draft isn't lost (see the small try/catch added in LessonHeader).
        throw err;
      } finally {
