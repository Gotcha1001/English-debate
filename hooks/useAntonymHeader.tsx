...
/**
 * Glue between <LessonHeader> and Convex for one Antonym Match set: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation). Same shape as
 * useQuickQuestionHeader, pointed at the `antonymSets` table.
 */
export function useAntonymHeader(id: Id<"antonymSets">) {
  const uploadHeaderImage = useAction(api.antonymsActions.uploadHeaderImage);
  const removeHeaderImage = useAction(api.antonymsActions.removeHeaderImage);
  const updateLearningObjective = useMutation(
    api.antonymsData.updateLearningObjective,
  );
