...
/**
 * Glue between <LessonHeader> and Convex for one Free Talking lesson: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation). Same shape as
 * useQuickQuestionHeader, pointed at the `lessons` table.
 */
export function useLessonHeader(id: Id<"lessons">) {
  const uploadHeaderImage = useAction(api.lessonsActions.uploadHeaderImage);
  const removeHeaderImage = useAction(api.lessonsActions.removeHeaderImage);
  const updateLearningObjective = useMutation(
    api.lessonData.updateLearningObjective,
  );
