...
/**
 * Glue between <LessonHeader> and Convex for one Grammar Breakdown set:
 ...
 * useQuickQuestionHeader, pointed at the `grammarSets` table.
 */
export function useGrammarHeader(id: Id<"grammarSets">) {
  const uploadHeaderImage = useAction(api.grammarActions.uploadHeaderImage);
  const removeHeaderImage = useAction(api.grammarActions.removeHeaderImage);
  const updateLearningObjective = useMutation(
    api.grammarData.updateLearningObjective,
  );
