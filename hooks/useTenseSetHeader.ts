...
/**
 * Glue between <LessonHeader> and Convex for one Tenses set: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation). Same shape as
 * useQuickQuestionHeader, pointed at the `tenseSets` table.
 */
export function useTenseSetHeader(id: Id<"tenseSets">) {
  const uploadHeaderImage = useAction(api.tensesActions.uploadHeaderImage);
  const removeHeaderImage = useAction(api.tensesActions.removeHeaderImage);
  const updateLearningObjective = useMutation(
    api.tensesData.updateLearningObjective,
  );
