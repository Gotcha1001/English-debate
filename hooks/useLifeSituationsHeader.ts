...
/**
 * Glue between <LessonHeader> and Convex for one life-situations set: image
 * upload/remove (Cloudinary, via actions) and the editable "what you'll
 * learn today" paragraph (plain mutation). Same shape as
 * useQuickQuestionHeader, pointed at the `lifeSituationSets` table.
 */
export function useLifeSituationHeader(id: Id<"lifeSituationSets">) {
  const uploadHeaderImage = useAction(
    api.lifeSituationsActions.uploadHeaderImage,
  );
  const removeHeaderImage = useAction(
    api.lifeSituationsActions.removeHeaderImage,
  );
  const updateLearningObjective = useMutation(
    api.lifeSituationsData.updateLearningObjective,
  );
