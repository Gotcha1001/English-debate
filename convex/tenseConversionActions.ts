...
// Every action checks ownership first via the public `getTenseConversionSet`
// query, which runs with the caller's Clerk identity and returns null for
// anyone else's set. Mirrors convex/quickQuestionsActions.ts.
...
): Promise<Doc<"tenseConversionSets">> {
  const set = await ctx.runQuery(
    api.tenseConversionData.getTenseConversionSet,
    {
      id,
    },
  );
...
export const uploadHeaderImage = action({
  args: { id: v.id("tenseConversionSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.tenseConversionData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.tenseConversionData.clearHeaderImage, {
      id,
    });
    return null;
  },
 ...
 * `tenseConversionData.deleteTenseConversionSet` mutation, which couldn't
 * reach Cloudinary. Use it with
 * `useDeleteSetAction(api.tenseConversionActions.deleteTenseConversionSet, "tense conversion set")`.
 */
export const deleteTenseConversionSet = action({
  args: { id: v.id("tenseConversionSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.tenseConversionData.deleteRow, { id });
    return null;
  },
});
