...
// Shared "header" fields, spread into every lesson/set table below.
// - learningObjective: AI drafts this ("What you'll learn today") at
//   generation time; the teacher can edit it afterward on the detail page.
// - headerImage: optional, uploaded by the teacher from the detail page
//   (after generation, not during). We store both the delivery `url` and
//   the Cloudinary `publicId` -- the publicId is required later to delete
//   the asset from Cloudinary when the lesson/set itself is deleted.
const headerFields = {
