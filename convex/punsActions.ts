...
// Every action checks ownership first via the public `getPunSet` query,
// which runs with the caller's Clerk identity and returns null for anyone
// else's set. Mirrors convex/wordRelationsActions.ts / grammarActions.ts.
...
async function requireOwnedSet(
  ...
  const set = await ctx.runQuery(api.punsData.getPunSet, { id });
  if (!set) throw new Error("Pun set not found.");
  return set;
}

/** Upload (or replace) the header image. Returns the new delivery URL. */
export const uploadHeaderImage = action({
  args: { id: v.id("punSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.punsData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.punsData.clearHeaderImage, { id });
    return null;
  },
});
 ...
 * `punsData.deletePunSet` mutation, which couldn't reach Cloudinary. Use it
 * with `useDeleteSetAction(api.punsActions.deletePunSet, "pun set")`.
 */
export const deletePunSet = action({
  args: { id: v.id("punSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.punsData.deleteRow, { id });
    return null;
  },
});
