...
// Every action checks ownership first via the public `get` query, which runs
// with the caller's Clerk identity and returns null for anyone else's set.
...
): Promise<Doc<"quickQuestionSets">> {
  const set = await ctx.runQuery(api.quickQuestionsData.get, { id });
...
export const uploadHeaderImage = action({
  args: { id: v.id("quickQuestionSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.quickQuestionsData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.quickQuestionsData.clearHeaderImage, {
      id,
    });
    return null;
  },
 ...
 * `deleteQuickQuestionSet` mutation, which couldn't reach Cloudinary.
 */
export const deleteSet = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.quickQuestionsData.deleteRow, { id });
    return null;
  },
});
