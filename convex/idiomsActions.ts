...
// Every action checks ownership first via the public `getIdiomSet` query,
...
async function requireOwnedSet(
  ctx: ActionCtx,
  id: Id<"idiomSets">,
): Promise<Doc<"idiomSets">> {
  const set = await ctx.runQuery(api.idiomsData.getIdiomSet, { id });
  if (!set) throw new Error("Idiom lesson not found.");
  return set;
}

/** Upload (or replace) the header image. Returns the new delivery URL. */
export const uploadHeaderImage = action({
  args: { id: v.id("idiomSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.idiomsData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.idiomsData.clearHeaderImage, { id });
    return null;
  },
});
 ...
 * `idiomsData.deleteIdiomSet` mutation, which couldn't reach Cloudinary.
 * Use it with
 * `useDeleteSetAction(api.idiomsActions.deleteIdiomSet, "idiom lesson")`.
 */
export const deleteIdiomSet = action({
  args: { id: v.id("idiomSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.idiomsData.deleteRow, { id });
    return null;
  },
});
