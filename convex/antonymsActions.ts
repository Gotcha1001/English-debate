...
// Every action checks ownership first via the public `getAntonymSet` query,
...
async function requireOwnedSet(
  ctx: ActionCtx,
  id: Id<"antonymSets">,
): Promise<Doc<"antonymSets">> {
  const set = await ctx.runQuery(api.antonymsData.getAntonymSet, { id });
  if (!set) throw new Error("Antonym set not found.");
  return set;
}

/** Upload (or replace) the header image. Returns the new delivery URL. */
export const uploadHeaderImage = action({
  args: { id: v.id("antonymSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.antonymsData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.antonymsData.clearHeaderImage, { id });
    return null;
  },
});
 ...
 * `antonymsData.deleteAntonymSet` mutation, which couldn't reach Cloudinary.
 * Use it with
 * `useDeleteSetAction(api.antonymsActions.deleteAntonymSet, "antonym set")`.
 */
export const deleteAntonymSet = action({
  args: { id: v.id("antonymSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.antonymsData.deleteRow, { id });
    return null;
  },
});
