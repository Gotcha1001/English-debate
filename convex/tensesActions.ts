...
// Every action checks ownership first via the public `getTenseSet` query,
...
async function requireOwnedSet(
  ctx: ActionCtx,
  id: Id<"tenseSets">,
): Promise<Doc<"tenseSets">> {
  const set = await ctx.runQuery(api.tensesData.getTenseSet, { id });
  if (!set) throw new Error("Tenses set not found.");
  return set;
}

/** Upload (or replace) the header image. Returns the new delivery URL. */
export const uploadHeaderImage = action({
  args: { id: v.id("tenseSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.tensesData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.tensesData.clearHeaderImage, { id });
    return null;
  },
});
 ...
 * `tensesData.deleteTenseSet` mutation, which couldn't reach Cloudinary.
 * Use it with `useDeleteSetAction(api.tensesActions.deleteTenseSet, "tenses set")`.
 */
export const deleteTenseSet = action({
  args: { id: v.id("tenseSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.tensesData.deleteRow, { id });
    return null;
  },
});
