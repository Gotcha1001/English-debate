...
// Every action checks ownership first via the public `getWordRelationSet`
// query, which runs with the caller's Clerk identity and returns null for
// anyone else's set. Mirrors convex/grammarActions.ts / idiomsActions.ts.
...
): Promise<Doc<"wordRelationSets">> {
  const set = await ctx.runQuery(api.wordRelationsData.getWordRelationSet, {
    id,
  });
  if (!set) throw new Error("Word relation set not found.");
  return set;
}

/** Upload (or replace) the header image. Returns the new delivery URL. */
export const uploadHeaderImage = action({
  args: { id: v.id("wordRelationSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.wordRelationsData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.wordRelationsData.clearHeaderImage, {
      id,
    });
    return null;
  },
...
/**
 * Delete the whole set AND its Cloudinary image. Replaces a plain
 * `wordRelationsData.deleteWordRelationSet` mutation (a mutation can't reach
 * Cloudinary). Use it with
 * `useDeleteSetAction(api.wordRelationsActions.deleteWordRelationSet, "word relations set")`.
 */
export const deleteWordRelationSet = action({
  args: { id: v.id("wordRelationSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.wordRelationsData.deleteRow, { id });
    return null;
  },
});
