...
// Every action checks ownership first via the public `getLifeSituationSet`
...
): Promise<Doc<"lifeSituationSets">> {
  const set = await ctx.runQuery(api.lifeSituationsData.getLifeSituationSet, {
    id,
  });
...
export const uploadHeaderImage = action({
  args: { id: v.id("lifeSituationSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.lifeSituationsData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.lifeSituationsData.clearHeaderImage, {
      id,
    });
    return null;
  },
 ...
 * `lifeSituationsData.deleteLifeSituationSet` mutation, which couldn't reach
 * Cloudinary. Use it with
 * `useDeleteSetAction(api.lifeSituationsActions.deleteLifeSituationSet, "scenario set")`.
 */
export const deleteLifeSituationSet = action({
  args: { id: v.id("lifeSituationSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.lifeSituationsData.deleteRow, { id });
    return null;
  },
});
