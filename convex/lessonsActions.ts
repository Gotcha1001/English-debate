...
// Every action checks ownership first via the public `getLesson` query, which
// runs with the caller's Clerk identity and returns null for anyone else's
// lesson. Mirrors convex/quickQuestionsActions.ts.
...
async function requireOwnedLesson(
  ...
  const lesson = await ctx.runQuery(api.lessonData.getLesson, { id });
...
export const uploadHeaderImage = action({
  args: { id: v.id("lessons"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.lessonData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.lessonData.clearHeaderImage, { id });
    return null;
  },
});

/**
 * Delete the whole lesson AND its Cloudinary image. Replaces the old
 * `lessonData.deleteLesson` mutation, which couldn't reach Cloudinary.
 * Use it with `useDeleteSetAction(api.lessonsActions.deleteLesson, "lesson")`.
 */
export const deleteLesson = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.lessonData.deleteRow, { id });
    return null;
  },
});
