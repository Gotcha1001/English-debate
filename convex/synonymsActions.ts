...
// Every action checks ownership first via the public `getSynonymSet` query,
...
async function requireOwnedSet(
  ctx: ActionCtx,
  id: Id<"synonymSets">,
): Promise<Doc<"synonymSets">> {
  const set = await ctx.runQuery(api.synonymsData.getSynonymSet, { id });
  if (!set) throw new Error("Synonym set not found.");
  return set;
}

/** Upload (or replace) the header image. Returns the new delivery URL. */
export const uploadHeaderImage = action({
  args: { id: v.id("synonymSets"), imageDataUri: v.string() },
  handler: async (ctx, { id, imageDataUri }): Promise<{ url: string }> => {
    ...
    try {
      await ctx.runMutation(internal.synonymsData.setHeaderImage, {
        id,
        url: uploaded.url,
...
export const removeHeaderImage = action({
  ...
  handler: async (ctx, { id }): Promise<null> => {
    ...
    await ctx.runMutation(internal.synonymsData.clearHeaderImage, { id });
    return null;
  },
});
 ...
 * `synonymsData.deleteSynonymSet` mutation, which couldn't reach Cloudinary.
 * Use it with
 * `useDeleteSetAction(api.synonymsActions.deleteSynonymSet, "synonym set")`.
 */
export const deleteSynonymSet = action({
  args: { id: v.id("synonymSets") },
  handler: async (ctx, { id }): Promise<null> => {
    const set = await requireOwnedSet(ctx, id);
    if (set.headerImage) {
    ...
    await ctx.runMutation(internal.synonymsData.deleteRow, { id });
    return null;
  },
});
