 ...
 * the header image, and grammarActions.ts uses this query as its ownership
...
// NOTE: the old `deleteGrammarSet` mutation is gone. A mutation can't reach
// Cloudinary, so deleting a set that has a header image would orphan the
// file. Use `api.grammarActions.deleteGrammarSet` (an action) instead -- it
...
export const updateLearningObjective = mutation({
  args: { id: v.id("grammarSets"), learningObjective: v.string() },
  handler: async (ctx, args) => {
    const me = await getCurrentUser(ctx);
...
// --- internal (called only from grammarActions.ts) ----------------------
